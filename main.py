...
def no_projects(message):
    bot.send_message(message.chat.id, '➡️У тебя пока нет проектов!\nМожешь добавить их с помошью команды /new_project')
...
def info_project(message, user_id, project_name):
    ...
    markup = InlineKeyboardMarkup()
    markup.row_width = 1
    markup.add(InlineKeyboardButton("Удалить", callback_data=f"deletethis:{project_name}"))
...
"""
➡️Вот команды которые могут тебе помочь:
/start - начало
/info - команды
